...
def setup_usos_authentication() -> str:
    return "\n".join([
        ...
        "5. Once provided, use the `get_oauth_request_token` tool with the found `base_url` and the user-provided `consumer_key` and `consumer_secret` to obtain an `oauth_token`, `oauth_token_secret`, and an `authorize_url`. Keep the `oauth_token_secret` in your context, you will need it later.",
